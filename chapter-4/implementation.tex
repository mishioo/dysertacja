...
Aby usprawnić ten proces, porównania wykonywane są w~obrębie tak zwanego ruchomego okna \---
  ...
  a~wszystkie te, które znajdują się poza oknem są automatycznie uważanie za~niepodobne.
...
\citeauthor{polavarapu17} szczegółowo wyłożył w~cytowanym podręczniku\sidecite{polavarapu17}
...
Implementację odpowiednich funkcji\sidenote{%
    Użyte do~tego wzory matematyczne cytuję w~adekwatnej sekcji dokumentacji programu \tesliper{}.
  } oparłem o~te właśnie wiadomości, a~także o~informacje
  dostarczone przez twórców programu Gaussian\sidecite{gaussian-uv}.
...
Program \tesliper{} udostępnia dwie, najpowszechniej w~tym celu używane funkcje \--- funkcję
  ...
  Gaussa, której zazwyczaj używa się w~przypadku widm elektronowych\sidecite{polavarapu17}.
...
\begin{enumerate}
  ...
  \item Należy wybrać funkcję oraz wartość połówkowej szerokości, które zostaną użyte
