...
Polega on na~przesunięciu centroidu obydwu struktur do~środka układu współrzędnych,
  wyznaczeniu ich macierzy kowariancji i~w~końcu obliczeniu macierzy optymalnej obrotu
...
Następnie należy obliczyć macierz kowariancji $H$ obydwu wyśrodkowanych struktur,
  reprezentowanych w~poniższym wzorze przez macierze $^0A$ oraz $^0B$.
...
\begin{displaymath}
  H = U\varSigma{}V^\intercal
\end{displaymath}

Stąd można już obliczyć macierz optymalnej obrotu $R$ zgodnie z~poniższym równaniem.
Zastosowanie tej macierzy na~macierzy reprezentującej analizowany konformer prowadzi
  do~optymalnego nałożenia go na~konformer odniesienia.

\begin{displaymath}
  R = V \begin{bmatrix}
    1 & 0 & 0 \\
    0 & 1 & 0 \\
    0 & 0 & sgn(det(VU^\intercal))
  \end{bmatrix} U^\intercal
\end{displaymath}
