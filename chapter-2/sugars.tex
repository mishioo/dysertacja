...
Zamiast tego, w~mieszaninie poreakcyjnej znalazłem związek, który formalnie jest produktem
  cykloaddycji cyklopentadienu do~iminy \refcmpd{glu-imine}.
Dwa diastereomery tego związku, oznaczone wspólnym numerem \refcmpd{glu-cp-condensed}
...
Reakcje typu Dielsa-Aldera iminy z~cyklopentadienem przebiegają łatwo w~warunkach
  kwaśnych\sidecite{gorbunova13} lub zasadowych\sidecite{maggini90}.
...
Autorzy postulowali w~tym przypadku uwolnienie cyklopentadienu z~kompleksu cyrkonu\sidenote{
    A~dokładnie z~\ch{Cp2ZrO}, powstającego po~redukcji amidu odczynnikiem Schwartza.
  } pod wpływem anionu cyjankowego i~jego dalszą reakcję z~aktywnym kationem
  iminiowym\sidecite{ulikowski16}.
...
Związek~\refcmpd{glu-cp-condensed} powstał w~tej reakcji w~niewielkiej ilości, otrzymanym
...
\begin{scheme}
  ...
  \caption{
    ...
    Cykloaddycja do~zwiąku \refcmpd{glu-cy-condensed} następuje pod nieobecność innych czynników
...
Obecność produktu \refcmpd{glu-cy-condensed} w~reakcji bez żadnego nukleofila, mogącego zastąpić
...
Takie założenie nasuwa jednak pytanie \--- dlaczego związek~\refcmpd{glu-cy} obserwuję
  tylko w~reakcji z~dodatkiem cyklopentadienu?
Inną możliwością jest powstawanie produktu~\refcmps{glu-cp-condensed} bezpośrednio w~wyniku
...
Choć takie rozważania i~spekulacje są na~pewno interesujące, zaczynają wykraczać poza
