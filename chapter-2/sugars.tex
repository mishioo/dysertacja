...
Dwie pierwsze są strukturami aromatycznymi \--- para elektronowa atomu azotu jest
  ...
  występują w~roztworach w~równowadze\sidecite[-1em]{kiselev11}.
\iupac{5\H-Tetrazol} jest natomiast niearomatyczny \--- tym samym mniej trwały,
  oraz zdecydowanie mniej powszechny\sidenote[][-1em]{
...
Można znaleźć doniesienia o~ich zastosowaniu w~reakcji aldolowej\sidecite[-1.5em]{hartikka04},
...
\subsection{Wieloskładnikowe reakcje typu reakcji Ugiego}\label{synthesis:sugars:ugi}
...
\subsection{Model i~optymalizacja}\label{synthesis:sugars:opt}
W~\citeyear{nenajdenko13} \citeauthor{nenajdenko13} pokazali, że możliwe jest przeprowadzenie
  reakcji azydo-Ugiego w~wariancie Joulli{\'e}-Ugiego\sidenote[][-2em]{%
...
Wychodząc z~wywiedzionego~z glukozy laktamu~\refcmpd{glu-lactam} i~stosując ustalone wcześniej,
  ...
  otrzymałem z~zadowalającą, jak na~pierwszą próbę, wydajnością diastereoizomeryczne tetrazolowe
...
\begin{table}
  ...
  \caption{%
    ...
    \textsuperscript{a}Stosunek diastereoizomerów \iupac{\cip{2R}} do~\iupac{\cip{2S}},
      wg wydajności wydzielonych produktów.
    \textsuperscript{b}Dodatek użyty w~nadmiarze.
    \textsuperscript{c}Imina wydzielona po etapie redukcji.
  }\label{tab:sugars-opt}
\end{table}

\pagebreak  % move marginfigure below on next page
...
\begin{table}
  ...
  \caption{%
    ...
    Różnice w~konfiguracji absolutnej na~nowym centrum stereogenicznym w~produktach wywiedzionych
      z~glukozy i~galaktozy dyskutuję w~sekcji \protect\secref{numeric:stereo}.
    \textsuperscript{a}Stosunek diastereoizomerów \iupac{\cip{2R}} do~\iupac{\cip{2S}},
      według wydajności wydzielonych produktów.
  }
...
Uzyskany na~drodze tych przemian związek~\refcmpd{glu-tet-tricyclo} posiada trzy pierścienie
  ...
  alkaloidów, być może aktywnych biologicznie\sidecite[-2em]{finsinger15}.

\begin{scheme*}
  \includesvg{ugi-tricyclo}
  \caption{%
    Synteza związku \refcmpd{glu-tet-tricyclo} o~strukturze podobnej do~alkaloidów 
      chinolizydynowych z~iminocukru \refcmpd{glu-tet.est},
...
\subsection{Ślepe zaułki}
Do syntezy tetrazolowych pochodnych iminocukrów próbowałem podejść również inaczej
...
Funkcjonalizacja anionem cyjankowym przebiega dość wydajnie, dając tylko jeden diastereoizomer
  \refcmpd{glu-cn}.
...
\begin{scheme*}
  ...
  \caption[][-1em]{
    Planowana metoda syntezy w~dwóch etapach \iupac{\N}\-/niepodstawionej tetrazolowej pochodnej
      iminocukru okazała się nieskuteczna ze~względu na~niepowodzenie w~cykloaddycji azydku.
  }
...
Niepowodzenie w~tej materii było niejakim zaskoczeniem, ponieważ przemiana tego typu jest
  znana i~wielokrotnie opisana w~literaturze\sidecite[-1em]{sharpless03}.
...
\begin{marginscheme}
  ...
  \caption{
    ...
    Na~górze addycja w~podwyższonej temperaturze, na~dole addycja katalizowana chinoliną.
  }
...
Reakcje te przedstawiam na~\cref{sch:azide-nucleophile}. 
...
Dwa diastereoizomery tego związku, oznaczone wspólnym numerem \refcmpd{glu-cp-di}
...
Konfigurację centrum stereogenicznego w~pozycji 2. ustaliłem za~pomocą eksperymentu \gls{noesy}.
...
Analiza \NMR*{} świadczy, że powstaje tylko tylko jeden diastereoizomer\sidenote{%
    Przynajmniej w~granicach dokładności metody pomiaru.} ale konfiguracji absolutnej
