...
Zastosowanie takich samych warunków nie przyniosło jednak pożądanego rezultatu.
Zamiast tego w~mieszaninie poreakcyjnej znalazłem związek, który prawdopodobnie jest produktem
  cykloaddycji cyklopentadienu do~iminy \refcmpd{glu-imine}.
Obydwa diastereomery tego związku, oznaczone wspólnym numerem \refcmpd{glu-cp}
  na~\cref{sch:cp-addition}, powstają w~proporcji 1:1 i~nie udało mi się ich rozdzielić
  \--- ani za pomocą chromatografii klasycznej, ani wysokociśnieniowej.
Osobny eksperyment pokazał, że obecność indolu nie jest potrzebna do~ich powstawania.
