...
Chemicy na nowo zainteresowali się też możliwościami, jakie daje addycja $\pi$-nukleofili do~aktywowanych amidów.
...
Cyklizacja sililowego eteru enolu \refcmpd{w:tashiromine-sub} pod wpływem bezwodnika triflowego i~\acrshort{dtbmp} (\acrlong{dtbmp}) i~hydroliza
  skutkuje powstaniem $\alpha,\beta$-nienasyconego $\beta$-aminoaldehydu \refcmpd{w:tashiromine-prod}, prekursora wspomnianego związku.
...
Addycja $\pi$-nukleofili (jak np.~\refcmpd{w:allyl}) do~aktywowanych amidów prowadzi do powstania wysoce reaktywnych intermediatów \refcmpd{w:pyridine-int},
...
\citeauthor{belanger15} wykorzystali tę technikę opracowując analogiczną do~reakcji Mannicha
  metodę syntezy $\beta$-aminoestrów \refcmpd{w:b-amino-esther}\autocite{belanger15}.
...
W~rezultacie otrzymują rozmaite $\beta$-aminoestry \refcmpd{w:b-amino-esther}, które można łatwo przekształcić w~nienaturalne aminokwasy\autocite{belanger15}.
...
\begin{scheme*}
  ...
  \caption{
    Synteza rozgałęzionych $\beta$-aminoestrów, niedostępnych przy użyciu reakcji Mannicha i~innych klasycznych metod.
...
\citeauthor{ghosez69} we~wspomnianej wcześniej w~przełomowej pracy\autocite{ghosez} pokazali użycie rozmaitych związków w~tej roli.
...
Z~amidów trzeciorzędowych powstają w~tej sekwencji przemian kationy iminiowe zdolne do przyłączenia kolejnej cząsteczki nukleofila,
  prowadząc finalnie do $\alpha,\alpha$-dwupodstawionych amin.
...
Pokazują, że stosując równomolową ilość odczynnika Grignarda i~manipulując temperaturą prowadzenia reakcji można przeprowadzić
  ten proces dwuetapowo, stosując inny nukleofil w~drugim etapie i~otrzymując aminę z~dwojgiem różnych podstawników w~pozycji $\alpha$.
