...
Używając bezwodnika triflowego (\ch{(CF3SO2)2O}) zamiast fosgenu,
  otrzymali triflową sól keteniminy poprzez \iupac{\a-triflo}enaminę~\refcmpd{w:bh-dime-cl-ketenimine}\sidecite{ghosez81}.
...
Powstaje wtedy triflan \iupac{\N-(trifluorometanosulfonoylo)piridyniowy}, który jest właściwym środkiem triflującym\sidecite{charette01}.
...
Addycja \textpi-nukleofili (jak np.~\refcmpd{w:allyl}) do~aktywowanych amidów prowadzi do powstania wysoce reaktywnych intermediatów \refcmpd{w:pyridine-int},
  które ulegają samoczynnej cyklizacji i~aromatyzacji, tworząc od razu pochodne pirydyny \refcmpd{w:pyridine} \--- w~jednym kroku syntetycznym\sidecite{movassaghi07}.
...
Niedawno opublikowane prace rozszerzają wachlarz podobnych przekształceń \iupac{\N-arylowych} amidów o~syntezę różnorodnych
  chinolin\sidecite{wezeman16,liang17} i~\iupac{2,3-dihydrochinolin}\sidecite{huang19} w~reakcji odpowiednio z~alkinami i~alkenami.
\begin{scheme}
  \centering
...
Dekadę przed powstaniem niniejszej dysertacji \citeauthor{xiao10} pokazali,
  że możliwa jest addycja odczynnika Grignarda do aktywowanego za~pomocą \ch{Tf2O} wiązania amidowego\sidecite{xiao10}.
...
Co więcej, stwierdzili, że można też zaadaptować ideę reduktywnego gaszenia reakcji, wspomnianą już wcześniej\sidecite{belanger15}.
...
Dopiero te ulegają addycji do aktywowanego amidu drugorzędowego,
  a~powstającą iminę \refcmpd{w:imine-keto} można zredukować lub poddać działaniu kolejnego nukleofila\sidecite{xiao12}.
...
Funkcja ta, jako bioizoster\footnote{%
    Więcej o~bioizosteryźmie, choć w~kontekście tetrazoli, można znaleźć w~rozdziale \secref{literature:tetrazole:bioisosterizm}.%
...
W~pierwszym podejściu do tego zagadnienia wykorzystali \ch{NaBH4} jako reduktor.
...
\citeauthor{xiang10} stwierdzili, że można w~ten sposób prowadzić redukcję amidów drugo- jak i trzeciorzędowych,
  ale selektywność metody nie jest idealna \--- estry również są przy jej zastosowaniu redukowane\sidecite{xiang10}.

Lepsze wyniki osiągnęli \citeauthor{huang16b} kiedy udało im się połączyć\sidecite{huang16b} opracowaną dotąd metodologię 
  z~podejściem wykorzystującym \gls{tmds} oraz katalityczną ilość \ch{B(C6F5)3}\sidecite{tan09, chadwick14, blondiaux14}.
...
Naukowcy z~grupy Maulide odkryli, że~amidy trzeciorzędowe mogą być aktywowane bezwodnikiem triflowym
  w~kierunku reakcji innego typu niż opisywane dotąd \--- w~kierunku formalnej \ch{C-H} aktywacji w~pozycji \textbeta{}.
...
Pierwszym z~takich przypadków była cyklizacja związku \refcmpd{w:nuno-claisen-substrate},
  zawierającego fragment eteru allilowego, której przebieg pokazuję na~\cref{maulide-claisen}\sidecite{madelaine10}.
Zamiast prowadzić do~powstania dwupierścieniowego związku \refcmpd{w:nuno-claisen-expectred},
...
Przerób wodny mieszaniny reakcyjnej pozwala wydzielić lakton \refcmpd{w:nuno-claisen-got},
  którego różne pochodne badacze otrzymali też w~dwóch późniejszych pracach\sidecite{peng12,padmanaban15}.
