...
Jego użycie nie jest jednak jedyną dostępną metodą \---
...
Związek ten został wprowadzony do~użytku w~chemii syntetycznej przez Jeffreya
  Schwartza\sidecite[-2em]{schwartz74},
...
Prace Buchwalda i~in. zapewniły dogodne rozwiązanie tego ostatniego problemu:
  ...
  przemywając mieszaninę produktów chlorkiem metylenu\sidecite[-4em]{buchwald87}.
...
Alternatywą jest generowanie odczynnika Schwartza \insitu,
  ...
  Red\-/Al\sidecite[-6em]{gibson87}, \ch{LiEt3BH}\sidecite[-3.5em]{lipshutz90},
...
Sam \schwartz{} jest nierozpuszczalny w~tych, jak i~w~większości innych rozpuszczalników
  ...
  rozpuszczają się już bardzo dobrze we wspomnianych \ch{CH2Cl2} i~\acrshort{thf}.
...
Przykładami takich modyfikacji mogą być pochodna z~pentametylowanym ligandem
  \ch{[(Cp^*)2Zr(H)Cl]}\sidecite[-12.5\baselineskip]{barger84} czy pochodna triflowa
  \ch{[Cp2Zr(H)OTf]}\sidecite[-11\baselineskip]{husgen97,luinstra85}.
...
\citeauthor{wailes70} postulowali polimeryczną strukturę tego związku\sidecite[-17em]{wailes70},
...
Więcej informacji przyniosły dopiero wykonane 40~lat później pomiary \NMR*{35,Cl}
  w~ciele stałym, według których \refcmpd{w:schwartz} ma raczej budowę dimeryczną,
  podobnie do~\ch{[Cp2Zr(H)Me]}\sidecite[-18em]{rossini09}.
Niedawno ostateczny tego dowód dostarczyli \citeauthor{jones19} \---
  ...
  obraz rzeczywistej struktury wodorku chlorocyrkonocenu~\refcmpd{w:schwartz}\sidecite[-19em]{jones19},
...
% TODO: check if izomerization or migration should be on scheme
\begin{scheme}
  \centering
...
Schwartz i in. pokazali, że alkilowe pochodne cyrkonocenu \refcmpd{w:zircono-alkane-terminal}
  mogą zostać przekształcone w~alkohole\sidecite{blackburn75}, związki
  karbonylowe\sidecite{bertelo75}, czy halogenki alkilowe\sidecite{hart75}.
...
Można zastosować je jako reagenty w~krzyżowym sprzęganiu katalizowanym kompleksami metali%
  \sidenote{Zazwyczaj palladu lub niklu.}, albo przeprowadzić z~ich użyciem reakcję
...
Autorzy opisywanej pracy zastosowali w~roli elektrofila też inne reagenty,
  niż \ch{BzCl}, na~przykład \ch{Ph2PCl}, \ch{[CH2=N+ Me2]Cl-}, czy \ch{TfOH}.
\begin{scheme}
  \centering
...
\citeauthor{pinheiro17} zastosowali tę metodę do~redukcji nasyconych i~nienasyconych pochodnych
  azalaktonów\sidecite[-1\baselineskip]{pinheiro17}.
...
Częściowa redukcja do~formamidu może zostać przeprowadzona dzięki wykorzystaniu łagodniejszego
  \ch{NaBH4}\sidecite[-1.5em]{stecko16}, ale metoda ta nie toleruje wielu grup funkcyjnych.
\begin{marginscheme}
...
Naukowcy z~tej samej grupy pokazali niedawno, że tioizocyjanki mogą być poddane analogicznej
  przemianie, co prowadzi do powstania pochodnych tioformamidu\sidecite[-2em]{pace19}.
\begin{scheme}
  \includesvg{isocyanide-pace}
  \caption{
    Przykład częściowej redukcji izocyjanku, i~następującej po niej funkcjonalizacji,
      z~użyciem odczynnika Schwartza generowanego \insitu{} z~\ch{Cp2ZrCl2} oraz
...
Sądzili, że niezbędna będzie wstępna enolizacja wiązania \ch{C=O} wodorkiem potasu,
...
Przedstawiona w~górnej części~\cref{sch:amide-reduction-zr} procedura była pierwszą
...
Z~przeprowadzonych później wnikliwych badań mechanistycznych\sidenote{%
    Badania te obejmowały analizę mieszanin reakcyjnych przy użyciu technik \NMR*,
      \NMR*{13,C} oraz IR, a~także eksperymenty z~użyciem związków znaczonych izotopami
      \ch{D} i~\ch{^{18}O}.
...
Przekształcenia te prowadzili generując \schwartz{} \insitu{} za~pomocą Red\-/Al w~\ch{CH2Cl2},
...
\citeauthor{katahara17} odkryli, że trzeciorzędowe \iupac{\N-sililoksyamidy}
  ...
  \schwartz{} oraz \gls{tfa}, jak obrazuje \cref{sch:nitrones-zr}\sidecite[-3em]{katahara17}.
...
\begin{marginscheme}[-4em]
  \includesvg{nitrones-zr}
  \caption{
    Prowadząca do~otrzymania nitronu redukcja amidów z~podstawnikiem \ch{-O-\acrshort{tbs}}
      na~atomie azotu.
  }
...
W~oparciu o~wcześniejsze prace zespołów Ganema\sidecite[-2em]{schedler96} i~Georg\sidecite{white00}
...
Pokazali, że różne \iupac{\N-acylowane} amidy ulegają przekształceniu do~odpowiednich wolnych
  ...
  w~pokojowej temperaturze\sidecite[-1em]{sultane14}.
...
Wyraźną poprawą zaowocowało zastosowanie procedury \insitu{} zaproponowanej przez
  Zhao i~Snieckusa\sidecite{zhao14}, ale autorzy zastosowali ją w~tylko jednym przykładzie.
\begin{scheme}
...
\subsection{Redukcja laktamów}\label{literature:schwartz:rings}
\Cref{sch:zr-ncac-add} obrazuje, że reakcja reduktywnej addycji izocyjanoacetatów
  ...
  \refcmpd{ncac-amide}\sidecite{zheng17}.
...
Laktam jest wobec niej redukowany selektywnie, a~powstający związek \refcmpd{co2et-iminoamine}
...
\citeauthor{dandepally13} zademonstrowali redukcję
  ...
  przeprowadzoną przy użyciu odczynnika Schwartza\sidecite[-4\baselineskip]{dandepally13}.
...
Wynik ten, zobrazowany na~\cref{sch:nboc-hydantoin-reduction-zr}, jest zgodny z~opisanymi
  powyżej doniesieniami o~redukcji \iupac{\N-Boc-}laktamów~\refcmpd{nboc-lactam}.
...
\citeauthor{lanza13} zauważyli, że \iupac{\N-alkoksykarbonylowe} pochodne
  ...
  ulega w~tych warunkach rozpadowi wiązania \ch{N-Bz},
  dając \iupac{izokazolidyn-3-on}~\refcmpd{isoxazolidinone}.
...
Przeprowadzone przez nich obliczenia \gls{dft} są zgodne z~wynikami wspomnianymi
  wcześniej\sidecite{wang10} w~kwestii przebiegu redukcji, która następuje przez przeniesienie
...
Bariera aktywacji tego procesu w~znacznej mierze zależy od~charakteru podstawnika
  ...
  i~\iupac{\N-karbamylowych}\sidecite[-1.5em]{lanza13}.
...
Jednym z~takich przykładów jest wspomniana wcześniej procedura syntezy
  $(\pm)$\-/gefirotoksyny\sidenote{\See{sch:gephyrotoxin}.}.
Innym \--- synteza kwasu kainowego \refcmpd{kainic-acid}, którą zaproponowali
  \citeauthor{xia01}\sidecite{xia01}.
...
\begin{scheme*}
  ...
  \caption[][6\baselineskip]{
    Cykliczny \refcmpd{nitrone-cyclic} i~makrocykliczny~\refcmpd{nitrone-macro} nitron
...
Jak widać na~\cref{sch:zr-deoxygenation}, metoda ta jest na~tyle selektywna, że nie redukuje
  karbonylu z~grupy estrowej, obecnej w~związku \refcmpd{5-lactam-ester}.
\begin{scheme}
...
Ponieważ mała stabilność cyklicznych imin utrudnia ich stosowanie w~praktyce laboratoryjnej,
  ...
  \iupac{2-podstawionych} pirolidyn i~piperydyn.
...
\begin{scheme}
  ...
  \caption{
    ...
    może wpływać struktura zarówno substratu, jak i~nukleofila (\protect\see{sch:gal-danishefsky}).
    }
  \label{sch:gal-allyl}
\end{scheme}

Inna praca\sidecite{furman14acs} pochodząca z~grupy Furmana dowodzi, że z~wykorzystaniem opracowanej
  metody można przeprowadzić cyklokondensację aktywowanego laktamu z~dienem
  Danishefskiego~\refcmpd{danishefsky}.
Proces biegnie poprzez tandemową reakcję Mannicha i~kondensację Michaela,
  a~\cref{sch:gal-danishefsky} przedstawia jego skrót na~przykładzie laktamu~\refcmpd{lactam-gal}.
Jest to dogodna metoda syntezy polihydroksylowanych chinolizydyn i~indolizydyn\sidenote[][-1em]{%
    W przypadku zastosowania laktamów odpowiednio sześcio- i~pięcioczłonowych w~roli substratu.
  }, układów obecnych w~znacznej liczbie biologicznie aktywnych alkaloidów
  i~iminiocukrów\sidecite{horne14}.
...
\begin{scheme}
  \includesvg{gal-danishefsky}
  \caption{
    Synteza polihydroksylowej pochodnej chinolizydyny \refcmpd{quinolizidine-gal}
      z~laktamu o~strukturze galaktopiranozy \refcmpd{lactam-gal}.
    Proces biegnie diastereoselektywnie, a na~kierunek addycji
      wpływa struktura zarówno substratu, jak i~nukleofila (\protect\see{sch:gal-allyl}).
  }
  \label{sch:gal-danishefsky}
\end{scheme}

Kolejne badania pokazały, że hydrocyrkonowanie laktamów może być łatwo połączone z~wieloskładnikową
  reakcją Joulli\'{e}-Ugiego, zapewniając dostęp do~polihydroksylowych, amidowych pochodnych kwasu
  pipekolinowego oraz proliny\sidecite{furman15}.
\Cref{sch:our-joullie-ugi} przedstawia tę przemianę na~przykładzie funkcjonalizacji
  laktamu~\refcmpd{lactam-rybo} wywiedzionego z~rybozy.
Związki o~takiej strukturze jak produkt \refcmpd{prolino-amide-rybo} wykazują aktywność biologiczną
  w~obrębie, między innymi, inhibicji enzymu, którego niewłaściwy poziom w~organizmie może
  prowadzić do~chorób lizosomalnych, jak choćby choroba Alzheimera\sidecite{ayers14}.
Możliwość połączenia tej procedury z~reakcją wieloskładnikową jest zaletą samą w~sobie \---
  ...
  atomową, o~której wspominałem już wcześniej\sidenote{%
    Patrz: przypis \ref{note:atom-economy}, str.~\pageref{note:atom-economy}.}.
\begin{scheme*}
  \includesvg{our-joullie-ugi}
  \caption{
...
Tematyka zespołu dotyczy nie tylko pochodnych cukrów \--- \citeauthor{ulikowski16} przedstawili
  metodę syntezy \iupac{2,3-podstawionych} indoli~\refcmpd{2-3-indole}
  z~izatyn\sidecite[3\baselineskip]{ulikowski16}.
Opiera się ona na~wykorzystaniu różnicy charakteru obydwu grup karboksylowych obecnych
  w~substracie~\refcmpd{isatin} do~jego regiospecyficznej funkcjonalizacji,
  zgodnie ze~\cref{sch:isatins}.
Izatyna \refcmpd{isatin}, poddana działaniu odczynnika Grignarda, reaguje wyłącznie w~pozycji~3,
  dając \iupac{3-podstawiony} oksoindol \refcmpd{oxindole}.
...
Interesującym jest, że proces hydrocyrkonowania biegnie wydajnie, mimo obecności grupy hydroksylowej
  w~pozycji~3 związku~\refcmpd{oxindole}.
Ostatnia porcja \gls{tfa} jest niezbędna do~przeprowadzenia odwodnienia do~finalnego
  indolu~\refcmpd{2-3-indole}.
\begin{scheme}
  \includesvg{isatins}
  \caption{
    Regiospecyficzna synteza \iupac{2,3-podstawionych} indoli~\refcmpd{2-3-indole}
      z~izatyn~\refcmpd{isatin}, wykorzystująca różnice w~reaktywności grup karbonylowych.
  }
  \label{sch:isatins}
\end{scheme}
  
Nie tylko wspomniana wcześniej mała stabilność imin jest przeszkodą w~ich wykorzystaniu w~praktyce
  laboratoryjnej, w niektórych przypadkach sam dostęp do nich jest trudny lub niemożliwy.
W~jednej z~ostatnich prac\sidecite[-1em]{czerwinski19} zespołu \citeauthor{czerwinski19} przywołują
  przykład imin, które najprościej byłoby otrzymać w~kondensacji aminy i~aldehydu
  trifluorooctowego~\refcmpd{cf3-aldehyde}.
Związek ten jest znany, ale w~warunkach atmosferycznych jest gazem, otrzymywanym przez ogrzewanie
  odpowiedniego hemiacetalu \refcmpd{cf3-hemiacetal} w~obecności silnego kwasu mineralnego albo
  tlenku fosforu \ch{(V)}, i~musi być użyty natychmiast ze względu na~tendencję do~polimeryzacji.
O~wiele prostsze jest przygotowanie pochodnych trifluoroacetamidu~\refcmpd{cf3-amide} \---
  stabilnych i~łatwo dostępnych z~kwasu trifluorooctowego~\refcmpd{cf3-acid}.
...
Na~dowód prawdziwości tej tezy, \citeauthor{czerwinski19} otrzymali szereg funkcjonalizowanych
  \iupac{\a-trifluoroamin}~\refcmpd{cf3-amine} z~amidów~\refcmpd{cf3-amide}.
\begin{scheme*}
  \includesvg{trifluoro}
  \caption{
